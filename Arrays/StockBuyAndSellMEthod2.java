public class StockBuyAndSellMEthod2 {
    public static int maxProfit(int price[]) {
        int maxProfit = 0;
        int minIndex = 0;

        //Calculate max profit in optimized way
        for (int priceIndex = 0; priceIndex < price.length; priceIndex++) {
            if (price[priceIndex] < price[minIndex]) {
                minIndex = priceIndex;
            } else {
                if ((price[priceIndex] - price[minIndex]) > maxProfit) {
                    maxProfit = (price[priceIndex] - price[minIndex]);
                }
            }
        }
        return maxProfit;
